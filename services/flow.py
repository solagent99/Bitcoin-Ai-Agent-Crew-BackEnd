...
class TweetProcessingFlow(Flow[TweetAnalysisState]):
    ...
    def __init__(self, twitter_service: TwitterService):
        ...
        self.tools_map_all = initialize_tools(
            Profile(
                id=UUID(
                    os.getenv(
                        ...
                        "ff163ed8-b084-4fb3-899c-c7d5e483afaf",
                    )
                ),
            ...
            agent_id=UUID(
                os.getenv(
                    "AIBTC_TWITTER_AGENT_ID", "01ee34a3-a38c-44df-97be-b857a1bd9843"
                )
            ),
            crewai=True,
        )
        ...
        self.account_name = os.getenv("AIBTC_TWITTER_ACCOUNT_NAME", "@aibtcdevagent")
    ...
    async def execute_twitter_stream(
        ...
        try:
            logger.info(
                f"Starting tweet stream processing for input: {input_str[:50]}..."
            )
            ...
            if (
                self.state.tweet_type == TweetType.TOOL_REQUEST
                and self.state.tool_request
            ):
                logger.info(
